...
const Charts = {
  ...
  bar(id, labels, datasets, opts={}) {
    ...
    const mkGrad=(hex)=>{
      ...
      g.addColorStop(0.5,h+'55');
      g.addColorStop(1,h+'08');
      return g;
    };
    const mkBg=(d,i)=>{
  ...
  donut(id, labels, data, opts={}) {
    ...
    State.charts[id]=new Chart(canvas,{
      ...
      data:{ labels, datasets:[{
        data,
        backgroundColor:dPal.map(c=>c+'80'),
        borderColor:dPal,
        borderWidth:2.5,
  ...
  scatter(id, datasets, opts={}) {
    ...
    State.charts[id]=new Chart(canvas,{
      type:'scatter',
      data:{ datasets:datasets.map((d,i)=>({ borderColor:C.PALETTE[i%C.PALETTE.length], backgroundColor:C.PALETTE[i%C.PALETTE.length]+'44', pointRadius:6, pointHoverRadius:9, ...d }))},
      options:cfg
    });
    canvas.addEventListener('click',()=>{ Sound.unlock(); Sound.chart(); },{passive:true});
    return State.charts[id];
